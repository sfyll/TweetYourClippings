...
if __name__ == '__main__':
    ...
    try:
        ...
        key = pgpy_create_key(pwd=pwd)
        write_pgp_enc_to_file(current_path, key)

    key_information =  {
        "Key":"",
        "Secret":"",
        'Other_fields': {'bearer_token': '', 'access_token': '', 'access_token_secret': '', "client_id": "", "client_secret":""}
    }


    add_keys_to_encrypted_file(current_path, "TWITTER", pwd, key_information, key)

    # encrypted = pgpy_encrypt(key, str(key_information))

    # path = os.path.expanduser("~") + "/Documents/dev/kindle_handler/"
